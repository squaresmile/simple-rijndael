...
pub trait EncryptDecryptCbc {
    fn encrypt(&self, iv: Vec<u8>, source: Vec<u8>) -> Result<CryptResult, &'static str>;
    fn decrypt(&self, iv: Vec<u8>, cipher: Vec<u8>) -> Result<CryptResult, &'static str>;
    fn x_or_block(&self, b1: &Vec<u8>, b2: &Vec<u8>) -> Vec<u8>;
}

impl<P> EncryptDecryptCbc for RijndaelCbc<P>
where
    P: Padding,
{
    #[inline(always)]
